...
const personalMessage = `Hey, birthday girl! ❤️

Happy Birthday! 🎂✨

Kabhi-kabhi life mein kuch moments bilkul ordinary hote hain, but baad mein wahi moments sabse memorable ban jaate hain.

Ek saal pehle ek random journey mein ek simple si mulaqat hui thi—bas ek chhoti si conversation, kuch awkward smiles, aur ek safar jo normally kuch hours ka tha.

Us waqt kahan pata tha ki woh chhota sa moment itni achhi memory ban jayega. ❤️

So, on your special day, bas itni si wish hai—

May this year bring you countless reasons to smile, beautiful memories to look back on, and people who make even the simplest moments feel special.

Keep smiling, keep shining, and have the happiest birthday! 🫶🏻🎂✨
...
// Skip voucher button
const skipVoucherBtn = document.getElementById('skip-voucher');
if (skipVoucherBtn) {
    skipVoucherBtn.addEventListener('click', () => {
        showSection('outro-screen');
        showToast('✨ Thank you for being here ✨');
        createConfetti();
    });
...
// Skip modal button
const skipModalBtn = document.getElementById('skip-modal');
if (skipModalBtn) {
...
if (voucherForm) {
    voucherForm.addEventListener('submit', (e) => {
        ...
        const contactMethodVal = document.getElementById('contact-method')?.value;
        const email = document.getElementById('email')?.value || '';
        const phone = document.getElementById('phone')?.value || '';
        
        const contact = contactMethodVal === 'email' ? email : phone;
        sendGiftResponse({
            coffeeResponse: "yes",
            coupon: {
                code: voucherType,
                description: `Contact via ${contactMethodVal}`,
                value: 0,
...
document.getElementById('replay-btn').addEventListener('click', () => {
    ...
    if (nextToGalleryBtn) nextToGalleryBtn.style.display = 'none';
    const nextFromPrincessBtn = document.getElementById('next-from-princess');
    if (nextFromPrincessBtn) nextFromPrincessBtn.style.display = 'none';
    const nextToMinigameBtn = document.getElementById('next-to-minigame');
    if (nextToMinigameBtn) nextToMinigameBtn.style.display = 'none';
    const skipVoucherBtn = document.getElementById('skip-voucher');
    if (skipVoucherBtn) skipVoucherBtn.style.display = 'none';
...
window.addEventListener('resize', () => {
    ...
    if (canvas && window.innerWidth <= 768) {
        ...
        const giftModal = document.getElementById('gift-modal');
        const wasActive = giftScreen && giftScreen.classList.contains('active');
        const modalActive = giftModal && giftModal.classList.contains('active');
        if (wasActive && !modalActive) {
            initScratchCard();
        }
    }
});
